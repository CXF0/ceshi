 ...
 * @version 3.1.0 [2026-05-03]
 * @desc 修复：
 *   - 证照附件上传改为 /common/upload/certificates（不再混入 contracts 目录）
 *   - 发票附件上传改为 /common/upload/invoices
 *   - 统一解析通用上传接口返回的 res.data.data.url 结构
 */
...
// ── 通用上传工具（调用 /common/upload/:folder）────────
async function uploadToFolder(file: File, folder: string): Promise<{ url: string; name: string; size: number }> {
  const formData = new FormData();
  formData.append('file', file);
  const res: any = await request.post(`/common/upload/${folder}`, formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
  // 后端统一返回 { code, data: { url, name, size } }
...
const ContractDetail: React.FC = () => {
  ...
  const handleInvoiceUpload = async (paymentId: string, file: File) => {
    ...
    try {
      const { url } = await uploadToFolder(file, 'invoices');
      await updatePayment(paymentId, { invoiceUrl: url } as any);
  ...
  // ✅ 证书附件上传 → certificates 目录
  const handleCertFileUpload = async (file: File) => {
    setCertFileUploading(true);
    try {
      const { url, name } = await uploadToFolder(file, 'certificates');
      setCertFileUrl(url);
      setCertFileList([{ uid: '-1', name, status: 'done', url }]);
      message.success('证书附件上传成功');
    } catch { message.error('上传失败'); }
    finally { setCertFileUploading(false); }
    return false;
  };
  ...
  const handleCertSubmit = async () => {
    ...
    try {
      ...
      message.success('证书录入成功，已同步至证书管理');
  ...
  const payColumns: ColumnsType<FinPaymentItem> = [
    { title: '阶段', dataIndex: 'phaseName', width: 80 },
    {
      title: '应收', dataIndex: 'amountDue', width: 80,
      render: v => <span style={{ color: '#cf1322', fontWeight: 400 }}>¥{Number(v).toLocaleString()}</span>,
    },
    {
      title: '实收', dataIndex: 'amountPaid', width: 80,
      render: (v, record) => (
    ...
    {
      title: '开票', dataIndex: 'isInvoiced', width: 70,
    ...
    {
      // ✅ 发票附件 → invoices 目录
      title: '发票', dataIndex: 'invoiceUrl', width: 100,
      render: (url: string, record: any) => (
        <Space size={2}>
          ...
          >
            <Button type="link" size="small" icon={<UploadOutlined />} loading={invoiceUploading[record.id]}>
              {url ? '替换' : '上传'}
            </Button>
          </Upload>
        </Space>
    ...
    <div style={{ padding: '0 0 40px' }}>
      ...
      <Card bordered={false} style={{ borderRadius: 12, marginBottom: 16 }}>
        <Row justify="space-between" align="middle">
          ...
          <Col>
            {statusCfg.next && (
              <PermButton perm="/contract:edit" type="primary" icon={<ArrowRightOutlined />} loading={statusUpdating} onClick={handleStatusNext}>
                {statusCfg.nextLabel}
              </PermButton>
            )}
          </Col>
      ...
      {/* ── 状态进度条 ── */}
      <Card bordered={false} style={{ borderRadius: 12, marginBottom: 16 }}>
        <Steps current={statusCfg.step} items={[
      ...
      <Row gutter={16}>
        ...
        <Col xs={24} lg={14}>
          ...
          <Card bordered={false} style={{ borderRadius: 12, marginBottom: 16 }}
            ...
            <Descriptions column={2} size="small" labelStyle={{ color: '#8c8c8c', width: 90 }}>
              <Descriptions.Item label="认证主体" span={2}>
                <span style={{ fontWeight: 600, color: '#1677ff', fontSize: 15 }}>{contract.customer?.name || '--'}</span>
              </Descriptions.Item>
              <Descriptions.Item label="认证类型" span={2}>
                {certInfo ? (
                  <span><Tag color="blue">{certInfo.parentName}</Tag><span style={{ fontWeight: 500 }}>{certInfo.typeName}</span></span>
                ) : contract.certType || '--'}
              </Descriptions.Item>
              ...
              <Descriptions.Item label="合同总额">
                <span style={{ color: '#cf1322', fontWeight: 700, fontSize: 15 }}>¥{Number(contract.totalAmount).toLocaleString()}</span>
              </Descriptions.Item>
              <Descriptions.Item label="回款方式">{paymentTypeMap[contract.paymentType] || contract.paymentType}</Descriptions.Item>
              <Descriptions.Item label="是否返款">
                {contract.isRefund === 1 ? <Tag color="magenta">有返款 ¥{Number(contract.refundAmount).toLocaleString()}</Tag> : <Tag color="default">无</Tag>}
              </Descriptions.Item>
              <Descriptions.Item label="签约日期">{contract.signedDate || '--'}</Descriptions.Item>
              <Descriptions.Item label="服务有效期">
                {contract.startDate && contract.endDate ? `${contract.startDate} 至 ${contract.endDate}` : '--'}
              </Descriptions.Item>
              <Descriptions.Item label="创建人">{contract.createBy || '--'}</Descriptions.Item>
              <Descriptions.Item label="备注" span={2}><span style={{ color: '#595959' }}>{contract.remark || '—'}</span></Descriptions.Item>
            </Descriptions>
          </Card>
          ...
          <Card bordered={false} style={{ borderRadius: 12, marginBottom: 16 }}
            ...
            extra={
              <Upload showUploadList={false} beforeUpload={handleUpload} accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.png,.zip">
                <Button type="dashed" size="small" icon={<UploadOutlined />} loading={uploading}>上传附件</Button>
              </Upload>
            }>
            {attachments.length === 0 ? (
              <div style={{ textAlign: 'center', color: '#bbb', padding: '20px 0' }}>暂无附件，点击右上角上传</div>
            ) : (
              <div>
                {attachments.map((att, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: 8, marginBottom: 8, background: '#fafafa', border: '1px solid #f0f0f0' }}>
                    <Space>
                    ...
                    <Space>
                      <Tooltip title="预览"><Button type="link" size="small" icon={<EyeOutlined />} onClick={() => window.open(att.url, '_blank')} /></Tooltip>
                      <Tooltip title="下载"><Button type="link" size="small" icon={<DownloadOutlined />} onClick={() => { const a = document.createElement('a'); a.href = att.url; a.download = att.name; a.click(); }} /></Tooltip>
            ...
            extra={
              <Button type="primary" ghost size="small" icon={<PlusOutlined />}
               style={{ color: '#ffffffff' }}
                onClick={() => { certForm.resetFields(); setCertFileUrl(''); setCertFileList([]); setCertModalOpen(true); }}>
                录入证书
              </Button>
            }>
            {contractCerts.length === 0 ? (
              <div style={{ textAlign: 'center', color: '#bbb', padding: '20px 0' }}>暂无关联证照，点击「录入证书」添加</div>
            ) : (
              <Table size="small" dataSource={contractCerts} rowKey="id" columns={certColumns} pagination={false} scroll={{ x: 600 }} />
            )}
          </Card>
        </Col>
        ...
        <Col xs={24} lg={10}>
          <Card bordered={false} style={{ borderRadius: 12, marginBottom: 16 }}>
            <Row gutter={16}>
              <Col span={8}><Statistic title={<span style={{ fontSize: 12 }}>合同总额</span>} value={contract.totalAmount} precision={2} prefix="¥" valueStyle={{ fontSize: 16, color: '#1677ff' }} /></Col>
              <Col span={8}><Statistic title={<span style={{ fontSize: 12 }}>已回款</span>} value={totalPaid} precision={2} prefix="¥" valueStyle={{ fontSize: 16, color: '#52c41a' }} /></Col>
              <Col span={8}><Statistic title={<span style={{ fontSize: 12 }}>待回款</span>} value={Math.max(0, contractTotal - totalPaid)} precision={2} prefix="¥" valueStyle={{ fontSize: 16, color: totalDue > totalPaid ? '#faad14' : '#52c41a' }} /></Col>
            </Row>
            <div style={{ marginTop: 12 }}>
              ...
              <Progress percent={paidRate} strokeColor={paidRate >= 100 ? '#52c41a' : '#71ccbc'} trailColor="#f0f0f0" showInfo={false} />
            </div>
          </Card>

          <Card bordered={false} style={{ borderRadius: 12 }}
            title={<Space><DollarOutlined style={{ color: '#71ccbc' }} />回款阶段记录<Badge count={payments.length} style={{ backgroundColor: '#71ccbc' }} /></Space>}
            extra={<PermButton perm="/contract:edit" type="primary" size="small" icon={<PlusOutlined />} onClick={() => openPayModal()}>新增阶段</PermButton>}>
            <Table dataSource={payments} columns={payColumns} rowKey="id" loading={payLoading} size="small" pagination={false} scroll={{ x: 620 }} locale={{ emptyText: '暂无回款记录，点击「新增阶段」添加' }} />
          </Card>
        </Col>
      </Row>

      {/* ── 回款 Modal ── */}
      <Modal title={editingPay ? '编辑回款记录' : '新增回款阶段'} open={payModalOpen} onOk={handlePaySubmit} onCancel={() => setPayModalOpen(false)} width={480} destroyOnClose okText="确认保存">
        <Form form={payForm} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item name="phaseName" label="阶段名称" rules={[{ required: true }]}><Input placeholder="如：首款、阶段二、尾款" /></Form.Item>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="amountDue" label="应收金额" rules={[{ required: true }]}><InputNumber style={{ width: '100%' }} precision={2} prefix="¥" min={0} /></Form.Item></Col>
            <Col span={12}><Form.Item name="amountPaid" label="实收金额"><InputNumber style={{ width: '100%' }} precision={2} prefix="¥" min={0} /></Form.Item></Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="paymentDate" label="收款日期"><DatePicker style={{ width: '100%' }} /></Form.Item></Col>
            <Col span={12}><Form.Item name="isInvoiced" label="是否已开票" valuePropName="checked"><Switch checkedChildren="已开票" unCheckedChildren="未开票" /></Form.Item></Col>
          </Row>
        </Form>
      </Modal>

      {/* ✅ 录入证书 Modal */}
      <Modal title="录入证书（关联当前合同）" open={certModalOpen} onOk={handleCertSubmit}
        onCancel={() => { setCertModalOpen(false); certForm.resetFields(); setCertFileUrl(''); setCertFileList([]); }}
        confirmLoading={certSubmitting} width={600} destroyOnClose okText="确认录入">
        <Form form={certForm} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item label="认证主体">
            <Input value={contract?.customer?.name || '--'} disabled style={{ background: '#fafafa', color: '#595959' }} />
          </Form.Item>
          <Form.Item label="认证类型">
            <Input value={certInfo ? `${certInfo.parentName} / ${certInfo.typeName}` : (contract?.certType || '--')} disabled style={{ background: '#fafafa', color: '#595959' }} />
          </Form.Item>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="certificate_number" label="证书编号" rules={[{ required: true, message: '请输入证书编号' }]}><Input placeholder="如：ZD-2026-001" /></Form.Item></Col>
            <Col span={12}><Form.Item name="issuer" label="颁发机构"><Input placeholder="颁发机构全称" /></Form.Item></Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}><Form.Item name="issue_date" label="颁发日期" rules={[{ required: true, message: '请选择颁发日期' }]}><DatePicker style={{ width: '100%' }} /></Form.Item></Col>
            <Col span={12}><Form.Item name="expiry_date" label="到期日期" rules={[{ required: true, message: '请选择到期日期' }]}><DatePicker style={{ width: '100%' }} /></Form.Item></Col>
          </Row>
          <Form.Item label="证书附件（可选）">
            <Upload showUploadList={true} fileList={certFileList} beforeUpload={handleCertFileUpload}
              onRemove={() => { setCertFileUrl(''); setCertFileList([]); }} maxCount={1} accept=".pdf,.jpg,.jpeg,.png">
              <Button icon={<UploadOutlined />} loading={certFileUploading}>上传证书图片/扫描件</Button>
            </Upload>
          </Form.Item>
        </Form>
      </Modal>
